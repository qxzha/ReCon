...
class PrecompDataset(data.Dataset):
    ...
    def __init__(self, data_path, data_split, vocab, opt=None, logger=None, tokenizer=None):
        self.vocab = vocab
        ...
        if data_split == 'dev' and opt.data_name != "now100k_precomp":
            self.length = 5000
            if 'cc152k' in opt.data_name:
                self.length = 1000
    ...
    def process_caption(self, caption, caption_enhance):
        ...
        if not enhance:
            if self.data_name == "now100k_precomp":
                tokens = [self.vocab("<start>"), ]
                tokens.extend(int(cap) for cap in caption.split(",") if cap)
...
def get_precomp_loader(data_path, data_split, vocab, opt, batch_size=100,
                       shuffle=True, num_workers=2, logger=None, tokenizer=None):
    if shuffle:
        dset = PrecompDataset(data_path, data_split, vocab, opt, logger, tokenizer)
...
def get_loaders(data_name, vocab, batch_size, workers, opt, logger):
    ...
    train_loader = get_precomp_loader(dpath, 'train', vocab, opt,
                                      batch_size, True, workers, logger, opt.tokenizer)
    # get the val_loader
    val_loader = get_precomp_loader(dpath, 'dev', vocab, opt,
                                    batch_size, False, workers, logger, opt.tokenizer)
    # get the test_loader
...
def get_test_loader(split_name, data_name, vocab, batch_size, workers, opt):
    # get the data path
    # data_path = "/data"
    data_path = "/home/quaner/datasets/NCR-data/data"
