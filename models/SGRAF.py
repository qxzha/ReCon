...
class SelfAttention(nn.Module):
    ...
    def forward(self, all_embs, n_img_regions, n_cap_words):
        new_all_embs = all_embs.detach().clone()[:(n_img_regions + n_cap_words), :]

        query = self.query(new_all_embs)
        key = self.key(new_all_embs)
        value = self.value(new_all_embs)

        scale = torch.sqrt(torch.tensor(self.embed_size, dtype=torch.float32))

        S_all = torch.matmul(query, key.transpose(-1, -2))

        attn_all = torch.softmax(S_all / scale, dim=1)

        attn_vv = attn_all[:n_img_regions, :n_img_regions]
        attn_ll = attn_all[-n_cap_words:, -n_cap_words:]
        attn_vl = attn_all[:n_img_regions, -n_cap_words:]
        attn_lv = attn_all[-n_cap_words:, :n_img_regions]

        pseudo_v_index = attn_vl.argmax(-1).detach().cpu().numpy().tolist()

        pseudo_vv_rows = [[i] * n_img_regions for i in pseudo_v_index]
        pseudo_vv_cols = [pseudo_v_index] * n_img_regions

        pseudo_attn_vv = nn.Softmax(dim=-1)(attn_ll.detach().clone()[pseudo_vv_rows, pseudo_vv_cols])

        iais_loss_v = self.KLDivLoss(attn_vv.log(), pseudo_attn_vv) + self.KLDivLoss(pseudo_attn_vv.log(), attn_vv)

        pseudo_l_index = attn_lv.argmax(-1).detach().cpu().numpy().tolist()

        pseudo_ll_rows = [[i] * n_cap_words for i in pseudo_l_index]
        pseudo_ll_cols = [pseudo_l_index] * n_cap_words

        pseudo_attn_ll = nn.Softmax(dim=-1)(attn_vv.detach().clone()[pseudo_ll_rows, pseudo_ll_cols])

        iais_loss_l = self.KLDivLoss(attn_ll.log(), pseudo_attn_ll) + self.KLDivLoss(pseudo_attn_ll.log(), attn_ll)

        iais_loss = iais_loss_v + iais_loss_l

        new_all_embs = torch.matmul(attn_all, value)
        new_all_embs = self.out(new_all_embs)

        all_embs[:(n_img_regions+n_cap_words), :] = new_all_embs

        return all_embs, iais_loss
...
class EncoderSimilarity(nn.Module):
    ...
    def __init__(self, embed_size, sim_dim, module_name='AVE', sgr_step=3):
        ...
        self.selfattention = SelfAttention(embed_size)
    ...
    def forward_relation(self, img_emb, cap_emb, cap_lens):
        sim_img = []
        n_image = img_emb.size(0)
    ...
    def forward_emb_glo(self, img_emb, cap_emb, cap_lens):
        n_captions = cap_emb.size(0)

        img_ave = torch.mean(img_emb, dim=1)
        img_glo = self.v_global_w(img_emb, img_ave)
        ...
        for i in range(n_captions):
            n_word = cap_lens[i]
            cap_i = cap_emb[i, :n_word, :].unsqueeze(0)
            cap_ave_i = torch.mean(cap_i, dim=1)
            cap_glo_i = self.t_global_w(cap_i, cap_ave_i)
            cap_emb_all.append(cap_glo_i)
        cap_glo = torch.cat(cap_emb_all, dim=0)

        return img_glo, cap_glo
